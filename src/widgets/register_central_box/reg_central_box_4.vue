<script>
import input_password from '@/shared/ui/inputs/imput_password.vue';
import btn_next from '@/shared/ui/buttons/button_next.vue'
import btn_no_account from '@/shared/ui/buttons/button_no_account.vue';
import btn_forgot_your_password from '@/shared/ui/buttons/button_forgot_your_password.vue';
import input_login_or_email from '@/shared/ui/inputs/input_login_or_email.vue';
import welcom_login from '@/widgets/register_text_under_header/welcom_login.vue';


export default{
    components:{
    input_password,
    btn_next,
    btn_no_account,
    btn_forgot_your_password,
    input_login_or_email,
    welcom_login
    }
}

</script>

<template>

<div className="w-1/2 flex justify-start
    absolute left-1/4 top-1/4">
    <welcom_login></welcom_login>
    </div>
       
    <div className="w-1/2 flex justify-center
    absolute left-1/4 top-72">
    <div className="h-full w-full flex flex-col relative left-1/3">
    <div className="h-28 w-9"></div>
    <div className="">
    <div className="">
    <input_login_or_email></input_login_or_email>
    </div>
    <div className="mt-5">
      <input_password></input_password>
    </div> 
    <a href=""><btn_next></btn_next></a>
    <div class="flex justify-end h-[38px] w-[379px]">
        <div class="flex flex-col">
          <a href="/register"><btn_no_account></btn_no_account></a>
          <a href="/register5"><btn_forgot_your_password></btn_forgot_your_password></a>
        </div>
    </div>
    </div>     
    </div>
    

    </div>

</template>

